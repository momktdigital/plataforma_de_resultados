...
### 🎓 Importação de matrícula por Excel (descontinuada aqui)
Chegou a existir aqui um protótipo de cadastro de alunos via planilha Excel/CSV com parsing 100% client-side (`admin/upload_alunos.php` + `admin/js/di_parser.js` + `admin/alunos_di_process.php`), mas ele nunca funcionou de ponta a ponta em produção: `alunos_di_process.php` grava as colunas `cod_perfil`/`status`/`periodo_letivo`/`periodo`/`turma` e uma tabela `cursos` que nenhuma migration deste app chegou a criar. Um esboço de controle de acesso por curso também ficou pela metade (`$_SESSION['admin_role']`/`admin_curso'` nunca são populados por `admin/login.php`).

Esse fluxo foi **reconstruído do zero, corrigido, no módulo [Avaliações](avaliacoes/README.md)** — agora processado no servidor (não mais no navegador), com as migrations que faltavam (colunas de `alunos` + tabela `cursos`) e o mesmo reconhecimento flexível de cabeçalho (`RA`/`Matricula`/`MatriculaAluno`, `Per. Letivo`, etc.). Os arquivos legados citados acima ficam mantidos só como referência histórica; **usar a tela `/alunos/importar` do Avaliações**, não este protótipo.
